...
class Handler(BaseHTTPRequestHandler):
    ...
    ALLOWED_HOST_SUFFIXES = (".ts.net",)  # tailnet hostnames
    ALLOWED_HOST_EXACT = {"hermes-cloud.tail5fb6d1.ts.net", "100.81.211.55", "localhost", "127.0.0.1", "::1", "[::1]", "hermes-cloud.tail5fb6d1.ts.net:8787", "100.81.211.55:8787", "localhost:8787", "127.0.0.1:8787", "[::1]:8787"}
    ...
    def _host_allowed(self) -> bool:
        ...
        if not host:
            return False
        candidate = host
        if candidate.count(":") == 1 and candidate.rpartition(":")[2].isdigit():
            candidate = candidate.rpartition(":")[0]
        if candidate in self.ALLOWED_HOST_EXACT:
            return True
        return any(candidate.endswith(sfx) for sfx in self.ALLOWED_HOST_SUFFIXES)
...
def main() -> None:
    ...
    # Independent timer/liveness monitoring remains a separate deployment gate.
    from api import wsbound
    # WHY (2026-09-04 16:54 crash): HERMES_WEBUI_AGENT_DIR is prepended to sys.path,
    # so bare "from tools.ws_memory_monitor" resolved to the AGENT tree's tools (regular
    # package with __init__) and shadowed this WebUI module (ModuleNotFoundError loop).
    # Load by absolute file path: unambiguous identity regardless of env/path order.
    import importlib.util as _ilu
    _spec = _ilu.spec_from_file_location(
        "tools.ws_memory_monitor", str(Path(__file__).parent / "tools" / "ws_memory_monitor.py"))
    _wsm = _ilu.module_from_spec(_spec)
    _spec.loader.exec_module(_wsm)
    MemoryMonitor, start_memory_monitor = _wsm.MemoryMonitor, _wsm.start_memory_monitor
    memory_stop = threading.Event()
    memory_state = Path(__file__).parent / "tools" / "webui-mem-state.json"
    memory_thread = None
    try:
        cgroup_relative = next(line.split(":", 2)[2] for line in
                               Path("/proc/self/cgroup").read_text().splitlines()
                               if line.startswith("0::"))
        # WHY: isolated test servers share source but must never publish their
        # unrelated cgroup samples over the production admission heartbeat.
        if cgroup_relative.endswith("/hermes-webui-home.service"):
            wsbound.PRESSURE_STATE = memory_state
            monitor = MemoryMonitor(Path("/sys/fs/cgroup") / cgroup_relative.lstrip("/"),
                                    state_path=memory_state, session_dir=SESSION_DIR)
            memory_thread = start_memory_monitor(monitor, memory_stop)
    except Exception:
        logger.exception("webui memory telemetry unavailable; admission fails closed")
