...
CONTINUATION_HANDOFF_GRACE_SECONDS = 0.25
...
def _is_continuation_session(parent: dict | None, child: dict | None) -> bool:
    ...
    separate child-session row. Compression persistence can record the child a
    few milliseconds before the parent end timestamp, so a narrow handoff grace
    preserves those continuations. Materially overlapping parent/child links
    remain child sessions.
    ...
    try:
        ...
        return child_started_at + CONTINUATION_HANDOFF_GRACE_SECONDS >= parent_ended_at
    except (TypeError, ValueError):
        return False
