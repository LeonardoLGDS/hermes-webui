...
def _close_rejected_write_connection(handler) -> None:
    if str(getattr(handler, 'command', '') or '').upper() not in {'POST', 'PUT', 'PATCH', 'DELETE'}:
        return
    try:
...
def check_auth(handler, parsed) -> bool:
    ...
    if parsed.path == '/api/auth/logout':
        if has_session:
            return True
        _close_rejected_write_connection(handler)
