...
def install_extension(id: object, download_url: object, sha256: object) -> Dict[str, Any]:
    ...
    for member_name in file_members:
        ...
        if not decoded or not _is_safe_relative_path(decoded, allow_leaf_dotfile=True):
...
def uninstall_extension(id: object) -> Dict[str, Any]:
    ...
    with _EXTENSION_STATE_LOCK:
        ...
        for rel_path in entry.get("files", []):
            if not _is_safe_relative_path(rel_path, allow_leaf_dotfile=True):
                continue
...
def _is_safe_relative_path(rel: str, *, allow_leaf_dotfile: bool = False) -> bool:
    ...
    for index, segment in enumerate(segments):
        if not segment or segment in (".", ".."):
            return False
        if segment.startswith(".") and not (allow_leaf_dotfile and index == len(segments) - 1):
            return False
    return True
