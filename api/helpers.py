...
def j(handler, payload, status: int=200, extra_headers: dict=None, *, pretty: bool = True) -> None:
    ...
    *extra_headers*: optional dict of additional headers to include
    ...
    # WHY: 09-01/02 OOM recurrence: retain wire bytes, never a second parsed
    # session graph; capture only after the route's profile/redaction checks.
    from api.wsbound import RESPONSE_CAPTURE
    capture = RESPONSE_CAPTURE.get()
    if capture is not None:
        capture(body, status)
    return send_json_bytes(handler, body, status, extra_headers)


def send_json_bytes(handler, body, status=200, extra_headers=None):
    """Send already-projected JSON using the same auth/security boundary."""
    # WHY: conditional polling should not serialize/redact unchanged metadata
    # again (09-03 repeated 38s response writes); validators remain private.
    if getattr(handler, 'path', '').split('?', 1)[0] == '/api/sessions' and status == 200:
        from api.wsbound import etag
        extra_headers = {**(extra_headers or {}), 'ETag': etag(body)}
    ...
    handler.send_header('Cache-Control', 'private, no-cache' if 'ETag' in (extra_headers or {}) else 'no-store')
