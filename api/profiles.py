...
def profile_env_for_background_worker(
    ...
    *,
    ...
    mirror_hermes_home_to_process = True
    ...
    try:
        ...
        if _home_override_mod is not None:
            try:
                ...
                mirror_hermes_home_to_process = False
            except Exception:
                _home_override_token = None
                _home_override_installed = False
        ...
        process_runtime_env = dict(safe_runtime_env)
        if not mirror_hermes_home_to_process:
            process_runtime_env.pop("HERMES_HOME", None)
        ...
        with _ENV_LOCK:
            ...
            os.environ.update(process_runtime_env)
            if mirror_hermes_home_to_process:
...
def profile_scope_for_detached_worker(
    ...
    An explicit default/root profile still binds request-profile TLS so a
    detached worker cannot inherit the process-wide active named profile. Only
    the legacy process-env mirroring remains limited to named profiles.
