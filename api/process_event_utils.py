...
# Inverse of the structured ``format_wakeup_prompt`` shapes pinned by
...
_ASYNC_DELEGATION_RE = re.compile(
    r"\A\[ASYNC DELEGATION (?P<batch>BATCH )?COMPLETE — "
    r"(?P<delegation_id>[^\]\n]+)\](?:\n|\Z)"
)
...
def wakeup_display_meta(text: Any) -> dict | None:
    ...
    Returns header-only metadata for process completions, watch matches, and
    async-delegation completion envelopes, or None when the text is not one of
    those pinned shapes. The output section stays in the message body, so the
    ...
    m = _ASYNC_DELEGATION_RE.match(body)
    if m:
        return {
            "type": "async_delegation",
            "task_id": m.group("delegation_id"),
            "batch": bool(m.group("batch")),
        }
    return None
