...
def _configured_model_ids(raw_models: object) -> list[str]:
    ...
    if isinstance(raw_models, dict):
        candidates = (
            ...
            and key not in {'__explicit_model_allowlist__', '__discovered_model_catalog__'}
        )
...
def _models_config_is_allowlist(raw_models: object, discovered: bool = False) -> bool:
    """Return whether ``models`` is an intentional model-ID allowlist."""
    if discovered or raw_models is None:
        return False
    if isinstance(raw_models, str):
        return bool(raw_models.strip())
    if isinstance(raw_models, dict):
        return False
    if isinstance(raw_models, (list, tuple)):
        return bool(_configured_model_ids(raw_models))
    return False


def _provider_models_discovered(provider_cfg: object) -> bool:
    ...
    return (
        isinstance(raw_models, dict)
        and raw_models.get('__discovered_model_catalog__') is True
    )
        ...
        if isinstance(_custom_providers_cfg, list):
            ...
            for _cp in _custom_providers_cfg:
                ...
                if _slug and _cp_base_url:
                    ...
                    _cp_has_configured_models = _models_config_is_allowlist(
                        _cp_configured_models,
                        _provider_models_discovered(_cp),
                    )
        ...
        if detected_providers:
            ...
            for pid in sorted(detected_providers):
                ...
                if pid == "openrouter":
                    ...
                    # List/string model configs are explicit local allowlists.
                    # Dict-shaped configs are per-model metadata, and catalogs
                    # persisted with models_discovered are also non-restricting.
                    if (
                        isinstance(provider_cfg, dict)
                        and _models_config_is_allowlist(
                            provider_cfg.get("models"),
                            _provider_models_discovered(provider_cfg),
                        )
                    ):
                        raw_models = _configured_model_options(provider_cfg["models"])
