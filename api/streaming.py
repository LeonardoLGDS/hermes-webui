...
def _mark_active_turn_checkpoint(message, identity, source=None):
    if (
        isinstance(message, dict)
        and message.get('role') == 'user'
        ...
        resolved_source = identity.get('source') or source or 'webui'
        stamp_message_source(
            message,
            resolved_source,
            active_turn_token=identity['token'],
        )
        if str(resolved_source).strip().lower() == 'fork':
...
def _settle_current_turn_boundary(previous_context, result_messages, identity, msg_text, source):
    ...
    if _checkpoint_idx is not None:
        ...
        if isinstance(identity.get('checkpoint'), dict):
            ...
            _mark_active_turn_checkpoint(existing_checkpoint, identity, source)
        return result_messages
...
def _bg_notify_mode() -> str:
    """'all' or 'concise' for background-process notifications.

    WHY: the WebUI historically always dumped the full process output into the
    transcript. Honor the agent display setting; anything except literal 'all'
    (including the default False) renders concise so long-runner wakeups stay
    one line. Mirrors the gateway's mode semantics.
    """
    try:
    ...
    value = None
    cur = cfg
    for part in "display.background_process_notifications".split("."):
        if not isinstance(cur, dict) or part not in cur:
            value = None
            break
        cur = cur[part]
        value = cur
    return "all" if value == "all" else "concise"
...
def _tool_result_is_error(raw) -> bool:
    """Return whether explicit structured tool-result fields report failure."""
    data = raw if isinstance(raw, dict) else None
    if data is None and isinstance(raw, str):
        text = raw.strip()
        if text.startswith('{') and len(text) <= _TOOL_RESULT_SNIPPET_MAX * 4:
            try:
...
# pending".
#
# Fix: keep the socket-level write timeout for backpressured tabs, and arm TCP
# keepalive/TCP_USER_TIMEOUT plus a non-consuming peer probe for half-open tabs.
# Either path raises a member of api.routes._CLIENT_DISCONNECT_ERRORS, so each
# SSE handler leaves its loop, runs `finally: unsubscribe`, and releases the OS
# thread. SessionChannel already supports reconnect + offline buffer, so no
# events are lost for a tab that comes back. Operators behind unusual proxies
# can tune the write/dead-peer deadline without code changes.
try:
...
def _sse_set_socket_option(connection, level, option_name, value):
    option = getattr(_socket, option_name, None)
    if option is None or not hasattr(connection, "setsockopt"):
        return
    try:
        connection.setsockopt(level, option, value)
    except (OSError, TypeError, ValueError):
        logger.debug("Failed to set SSE socket option %s", option_name, exc_info=True)


def _sse_set_write_deadline(handler, seconds=None):
    """Best-effort: arm write and dead-peer deadlines on an SSE handler.
    ...
    try:
        ...
        _sse_set_socket_option(conn, _socket.SOL_SOCKET, "SO_KEEPALIVE", 1)
        if hasattr(_socket, "TCP_KEEPIDLE"):
            _sse_set_socket_option(conn, _socket.IPPROTO_TCP, "TCP_KEEPIDLE", 10)
            _sse_set_socket_option(conn, _socket.IPPROTO_TCP, "TCP_KEEPINTVL", 5)
            _sse_set_socket_option(conn, _socket.IPPROTO_TCP, "TCP_KEEPCNT", 3)
        elif hasattr(_socket, "TCP_KEEPALIVE"):
            _sse_set_socket_option(conn, _socket.IPPROTO_TCP, "TCP_KEEPALIVE", 10)
        try:
            peer_timeout_ms = max(1000, int(float(seconds) * 1000))
        except (TypeError, ValueError, OverflowError):
            peer_timeout_ms = None
        if peer_timeout_ms is not None:
            _sse_set_socket_option(
                conn,
                _socket.IPPROTO_TCP,
                "TCP_USER_TIMEOUT",
                peer_timeout_ms,
        ...
        logger.debug("Failed to arm SSE connection deadlines", exc_info=True)


def _sse_peer_is_dead(handler) -> bool:
    connection = getattr(handler, "connection", None)
    if connection is None:
        return False
    try:
        if hasattr(connection, "fileno") and connection.fileno() < 0:
            return True
    except (OSError, TypeError, ValueError):
        return False
    try:
        if hasattr(connection, "getsockopt"):
            socket_error = connection.getsockopt(_socket.SOL_SOCKET, _socket.SO_ERROR)
            if socket_error:
                return True
    except (OSError, TypeError, ValueError):
        pass
    try:
        readable, _, exceptional = _select.select([connection], [], [connection], 0)
    except (OSError, TypeError, ValueError):
        return False
    if exceptional:
        return True
    if not readable or not hasattr(connection, "recv"):
        return False
    flags = getattr(_socket, "MSG_PEEK", 0) | getattr(_socket, "MSG_DONTWAIT", 0)
    try:
...
def _sse_write_heartbeat(handler, payload=b": keepalive\n\n") -> None:
    if _sse_peer_is_dead(handler):
        raise ConnectionResetError("SSE peer is no longer connected")
        ...
        try:
            ...
            def on_tool(*cb_args, **cb_kwargs):
                ...
                if event_type == 'tool.completed':
                    result_is_error = bool(cb_kwargs.get('is_error', False)) or _tool_result_is_error(
...
def _handle_chat_steer(handler, body: dict) -> bool:
    ...
    if not cached:
        ...
        if active_stream_id:
            # A persisted active_stream_id can be a live gateway run even when the
            # webui-local STREAMS/ACTIVE_RUNS tables no longer know it (a webui
            # restart wipes those tables while the gateway keeps running). Queueing
            # the steer text for the next turn is safe on both sides: a live run
            # consumes the queue, a dead run simply makes it the next turn. So
            # return the queue-safe gateway_steer_queued fallback for ANY persisted
            # active_stream_id instead of the old "no_cached_agent" steer failure
            # that scared the user and left the message stuck in the composer
            # (nesquena/hermes-webui #5723).
            ...
            return j(handler, {"accepted": False, "fallback": "gateway_steer_queued",
                               "stream_id": active_stream_id})
        # No cached agent and no persisted stream id for this session, so nothing
        # is running: the frontend treats this as a dead run and recovers by
        # sending the message as a new turn.
