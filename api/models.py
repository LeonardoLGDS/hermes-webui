...
_CLI_SESSIONS_CACHE_TTL_SECONDS = 30.0
_CLI_SESSIONS_ALL_PROFILES_IMPORT_LIMIT = 300
# state.db is multi-GB; rebuilding every 5s stalls /api/sessions for seconds at
# a time (verified 2026-08-24: 8-27s requests); 30s keeps the list fresh
# without the stall.
...
def _guard_persisted_exact_message_replays(messages) -> tuple[list, int]:
    original = list(messages or [])
    guarded = []
    canonical_by_identity: dict[tuple[str, str], set[str]] = {}
    ...
    for message in original:
        if not isinstance(message, dict):
            ...
            continue
        message_id = _identity_component(message.get("id"))
        timestamp = _identity_component(message.get("timestamp"))
        if timestamp is None:
            timestamp = _identity_component(message.get("_ts"))
        ...
        try:
            ...
            logger.warning(
                "persisted replay guard disabled: canonicalization failed"
            )
            return original, 0
        identity = (message_id, timestamp)
        canonical_messages = canonical_by_identity.setdefault(identity, set())
        if canonical in canonical_messages:
            continue
        canonical_messages.add(canonical)
        guarded.append(message)
    return guarded, len(original) - len(guarded)
...
class Session:
    ...
    def save(self, touch_updated_at: bool = True, skip_index: bool = False) -> None:
        ...
        original_messages = list(self.messages or [])
        messages_to_persist, replay_rows_removed = _guard_persisted_exact_message_replays(
            original_messages
        )
        ...
        payload = _encode_payload(messages_to_persist)
        ...
        try:
            if self.path.exists():
                ...
                try:
                    ...
                    existing_msg_count = math.inf  # corrupt → always back up
                incoming_msg_count = len(messages_to_persist)
                if (
                ...
                if existing_msg_count > incoming_msg_count:
                    ...
                    try:
                        ...
                        if replay_rows_removed:
                            logger.warning(
                                "persisted replay guard skipped for %s because shrink backup failed",
                                self.session_id,
                            )
                            messages_to_persist = original_messages
                            replay_rows_removed = 0
                            payload = _encode_payload(messages_to_persist)
        except OSError:
            if replay_rows_removed:
                logger.warning(
                    "persisted replay guard skipped for %s because existing sidecar could not be inspected",
                    self.session_id,
                )
                messages_to_persist = original_messages
                replay_rows_removed = 0
...
def get_session(sid, metadata_only=False):
    ...
    from api.wsbound import DERIVED_READ
    if DERIVED_READ.get():
        # WHY: bounded wire caching cannot help if browsing every old session
        # also retains its parsed graph (09-01/02 OOM). Keep existing live/dirty
        # objects authoritative but do not populate the writer cache on reads.
        return _resolve_session(sid, metadata_only=metadata_only,
                                promote_cache=False, cache_on_miss=False)
    return _resolve_session(sid, metadata_only=metadata_only)
...
def _refresh_index_rows_from_sidecar_metadata(
    ...
    *,
    ...
    for session in sessions:
        ...
        except (Exception, MemoryBudgetExceeded) as exc:
            # WHY: this overlay is best-effort. A corrupt/unexpectedly large
            # legacy sidecar must keep its stale index row, not turn the whole
            # sidebar into a permanent 503 (MemoryBudgetExceeded is deliberately
            # a BaseException, so the existing Exception handlers miss it).
            logger.warning("Skipping bad sidecar metadata refresh for %s: %s", sid, exc)
            out.append(session)
            continue
        if not sidecar:
            out.append(session)
...
def _parse_claude_code_jsonl(path: Path, *, max_messages: int = CLAUDE_CODE_MAX_MESSAGES_PER_FILE) -> tuple[list[dict], str | None, float | None, float | None]:
    ...
    last_ts = None
    try:
        from api.wsbound import READ_BUDGET, read_source_text
        import io
        bounded_source = io.StringIO(read_source_text(path)) if READ_BUDGET.get() is not None else None
        with (bounded_source if bounded_source is not None else path.open('r', encoding='utf-8', errors='replace')) as fh:
            for line in fh:
        ...
        if all_profiles:
            ...
            for idx, (ctx_home, ctx_db_path, ctx_profile) in enumerate(contexts):
                load_kwargs = {
                    ...
                    'visible_session_limit': _CLI_SESSIONS_ALL_PROFILES_IMPORT_LIMIT,
    ...
    *,
    ...
    try:
        with closing(open_state_db_readonly(db_path)) as conn:
            ...
            from api.wsbound import READ_BUDGET, reserve_sql_rows
            if READ_BUDGET.get() is not None:
                # WHY: byte measurement and fetch must see one SQLite snapshot;
                # a concurrent append must not outgrow the admitted reservation.
                conn.execute("BEGIN")
            ...
            reserve_sql_rows(
                ...
                f"session_id IN ({placeholders}) {since_clause} {active_clause}",
                params[:-1] if limit_clause else params,
            )
            if limit_clause:
                cur.execute(f"""
                    SELECT * FROM (
