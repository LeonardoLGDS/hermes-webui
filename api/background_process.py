...
def bg_status_for_session(session_id: str) -> dict:
    """Return live process-registry status for a WebUI session."""
    ...
    try:
        ...
        rows = process_registry.list_sessions(session_key=sid)
        for row in rows:
            exited = row.get("status") == "exited"
            exit_code = row.get("exit_code") if exited else None
            process = {
            ...
            output = row.get("output_preview") if exited else None
            if output:
                process["output"] = output
            processes.append(process)
        active = any(process["state"] == "running" for process in processes)
    except Exception:
        # The registry is supplied by the optional Hermes Agent runtime. Sidebar
        # rendering must fail closed when it is unavailable or unreadable.
        logger.debug(
            "background process-status lookup failed for session %s",
            sid,
            exc_info=True,
        )
        active = False
        processes = []
    return {"session_id": sid, "active": active, "processes": processes}


def bulk_bg_active_by_session() -> set[str]:
    """Return WebUI session IDs with running registry processes.

    ``list_sessions()`` refreshes every detached process, so the sidebar must
    call it once per response rather than once per session row. Process rows do
    not expose ``session_key`` in their serialized form; the one direct registry
    lookup per *running* row recovers that spawn-time owner without another
    full registry listing. An unmapped key is retained as a direct owner
    candidate and harmlessly intersects with the session-row ID set in routes.
    """
    active: set[str] = set()
    try:
        from tools.process_registry import process_registry
        ...
        with _cfg.PROCESS_SESSION_INDEX_LOCK:
            owners = dict(_cfg.PROCESS_SESSION_INDEX)
        for row in rows:
            if not isinstance(row, dict) or row.get("status") == "exited":
                continue

            process_id = str(row.get("session_id") or "")
            process_lookup = getattr(process_registry, "get", None)
            process = (
                process_lookup(process_id)
                if process_id and callable(process_lookup)
                else None
            )
            if process is not None and getattr(process, "exited", False):
                continue

            session_key = str(
                getattr(process, "session_key", "") or row.get("session_key") or ""
            )
            session_id = owners.get(session_key) or session_key
            if session_id:
                active.add(session_id)
        return active
    except Exception:
        # Sidebar rendering is read-only and must fail closed if the optional
        # Hermes Agent registry is unavailable or returns malformed state.
        logger.debug("bulk background process-status lookup failed", exc_info=True)
        return set()


def _emit_bg_status_for_session(session_id: str) -> None:
    """Publish the canonical per-session background status frame."""
    sid = str(session_id or "")
    if not sid:
        return
    try:
        _emit_to_session_streams(sid, "bg_status", bg_status_for_session(sid))
    except Exception:
        logger.debug(
...
def _record_async_delegation_accepted(
    ...
    *,
    ...
    _emit_bg_status_for_session(session_id)
