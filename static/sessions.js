...
function _isSessionLocallyStreaming(s) {
  ...
  // For the active session, rely on S.busy to indicate an ongoing live turn.
  ...
  // Background-only work is not a live turn: it paints the sidebar's hollow
  // dot instead and must not keep streaming predicates/polling alive.
  return Boolean(isActive && S.busy);
}
...
if (typeof window !== 'undefined') {
  window._sessionBgState = window._sessionBgState || {};
}
...
function _sessionBgStateForRow(s) {
  const sid = s && s.session_id;
  const state = sid && window._sessionBgState && window._sessionBgState[sid];
  return {active: Boolean(state ? state.active : s && s.bg_active)};
}

function _seedSessionBgStateFromRow(row) {
  if (!row || !row.session_id) return;
  const state = {active: Boolean(row.bg_active)};
  window._sessionBgState[row.session_id] = state;
  row.bg_active = state.active;
}

// WHY: live bg_status edges should repaint only affected dots, not rebuild the sidebar.
function _patchSessionStatusDot(sid) {
  if (!sid || typeof document === 'undefined') return false;
  const rows = _sessionStatusInputRows();
  const rowsById = new Map(rows.map(row => [row.session_id, row]));
  const source = rowsById.get(sid);
  if (!source) return false;
  const sourceKey = _sidebarLineageKeyForRow(source) || sid;
  let patched = false;
  document.querySelectorAll('.session-item[data-sid]').forEach(el => {
    const row = rowsById.get(el.dataset.sid);
    if (!row) return;
    const rowKey = _sidebarLineageKeyForRow(row) || row.session_id;
    if (row.session_id !== sid && rowKey !== sourceKey) return;
    const titleRow = el.querySelector('.session-title-row');
    if (!titleRow) return;
    const dotRecord = _sessionDotStateForRow(row);
    let dot = titleRow.querySelector('.session-dot');
    if (!dotRecord || dotRecord.state === 'idle') {
      if (dot) dot.remove();
    } else {
      if (!dot) {
        ...
        const anchor = titleRow.querySelector('.session-source-chip') || titleRow.querySelector('.session-time');
        titleRow.insertBefore(dot, anchor || null);
      }
      dot.dataset.dotState = dotRecord.state;
      dot.title = _SESSION_DOT_TITLES[dotRecord.state] || 'Session status';
    }
    el.dataset.sessionStatus = dotRecord ? dotRecord.bucket : 'idle';
    const showArc = dotRecord && window._sessionStatus
      && typeof window._sessionStatus.showsRunningArc === 'function'
      && window._sessionStatus.showsRunningArc(dotRecord.state);
    el.classList.toggle('streaming', Boolean(showArc));
    patched = true;
  });
  return patched;
}

function _updateSessionBgState(sid, state) {
  if (!sid) return;
  const next = {active: Boolean(state && state.active)};
  const previous = window._sessionBgState[sid] || {active: false};
  window._sessionBgState[sid] = next;

  const updateRow = row => {
    if (row && row.session_id === sid) row.bg_active = next.active;
  };
  if (Array.isArray(_allSessions)) _allSessions.forEach(updateRow);
  if (Array.isArray(_sidebarReferenceSessions)) _sidebarReferenceSessions.forEach(updateRow);
  if (S.session && S.session.session_id === sid) S.session.bg_active = next.active;
  _noteSessionRowsForStatus();
  if (previous.active !== next.active && !_patchSessionStatusDot(sid)) renderSessionListFromCache();
}
if (typeof window !== 'undefined') window._updateSessionBgState = _updateSessionBgState;

function _sessionStatusInputRows(extraRows) {
  const rowsById = new Map();
  const addRows = rows => {
    if (!Array.isArray(rows)) return;
    for (const row of rows) {
      if (row && row.session_id) rowsById.set(row.session_id, row);
    }
  };
  // Reference rows preserve lineage context absent from the active list; full,
  // ephemeral, and finally attached render rows win the dedupe.
  addRows(_sidebarReferenceSessions);
  addRows(_allSessions);
  addRows(_sessionRowsWithActiveEphemeralSession(_allSessions));
  addRows(extraRows);
  return Array.from(rowsById.values());
}

function _noteSessionRowsForStatus(extraRows) {
  const status = window._sessionStatus;
  if (!status || typeof status.noteSessionRowsUpdated !== 'function') return false;
  const rows = _sessionStatusInputRows(extraRows).map(row => Object.assign({}, row, {
    // S4 is storage-free by contract. Resolve the sidebar's authoritative
    // unread value at this boundary instead of duplicating its local cache.
    unread: _hasUnreadForSession(row),
  }));
  return status.noteSessionRowsUpdated(rows, {
    session: S.session,
    busy: Boolean(S.busy),
  });
}
  ...
  deleteBtn.onclick=async()=>{
    ...
    try{
      const results=await Promise.all(ids.map(async sid=>{
        ...
        if(window._sessionStatus&&window._sessionStatus.pruneSession) window._sessionStatus.pruneSession(sid);
        return {response,session:sessionsById.get(sid)||null};
      }));
      const retainedCount=_worktreeResponseCount(results);
...
function _applySessionListPayload(sessData, projData, opts){
  ...
  _sidebarReferenceSessions = Array.isArray(sessData.sidebar_reference_sessions)
    ...
    : [];
  serverSessions.forEach(_seedSessionBgStateFromRow);
  _sidebarReferenceSessions.forEach(_seedSessionBgStateFromRow);
...
async function refreshActiveSessionIfExternallyUpdated(reason){
  ...
  try{
    // WHY: identify automatic reads so 09-03's polling/throttle feedback loop
    // can be shed without refusing a human's focus/click request.
    const data = await api(`/api/session?session_id=${encodeURIComponent(sid)}&messages=0&resolve_model=0`,{
      timeoutToast:false,
      headers:(reason||'poll')==='poll'?{'X-WebUI-Poll':'visible'}:{}
    });
...
const _SESSION_DOT_TITLES={
  draft:'Draft conversation',
  unread:'Unread completion',
  background:'Background work',
  working:'Working',
  stalled:'Working, waiting for update',
  'needs-input':'Needs your input',
};

function _sessionDotStateForRow(s){
  if(!s||!window._sessionStatus||!window._sessionDotState) return null;
  const key=_sidebarLineageKeyForRow(s)||s.session_id;
  const record=key&&window._sessionDotState[key];
  if(!record||typeof record.state!=='string') return null;
  const bucket=window._sessionStatus.sessionStatusBucket
    ?window._sessionStatus.sessionStatusBucket(record.state)
    :'idle';
  return {state:record.state,bucket};
}
...
function renderSessionListFromCache(){
  ...
  // Refresh from attached/collapsed rows before paint so lineage parents inherit
  // child claims through their shared lineage-tip key.
  _noteSessionRowsForStatus(sessions);
  ...
  function _renderOneSession(s, isPinnedGroup=false){
    ...
    const dotRecord=_sessionDotStateForRow(s);
    const statusShowsArc=dotRecord&&window._sessionStatus
      &&typeof window._sessionStatus.showsRunningArc==='function';
    // ownStreaming remains the semantic streaming value. This paint-only branch
    // lets S4 suppress the spinner for background/unread/draft/needs-input while
    // working/stalled reuse the existing row spinner.
    const paintStreaming=statusShowsArc
      ?window._sessionStatus.showsRunningArc(dotRecord.state)
      :isStreaming;
    ...
    el.dataset.sessionStatus=dotRecord?dotRecord.bucket:'idle';
    el.className='session-item'+(isActive?' active':'')+(isActive&&S.session&&S.session._flash?' new-flash':'')+(s.archived?' archived':'')+(paintStreaming?' streaming':'')+(hasUnread?' unread':'')+(attention?' needs-attention':'')+attentionClass;
    ...
    const hasAttentionState=paintStreaming||hasUnread||Boolean(attention);
    ...
    if(dotRecord&&dotRecord.state!=='idle'){
      ...
      dot.className='session-dot';
      dot.dataset.dotState=dotRecord.state;
      dot.title=(dotRecord.state==='needs-input'&&attention&&attention.title)
        ||_SESSION_DOT_TITLES[dotRecord.state]
        ||'Session status';
      dot.setAttribute('aria-hidden','true');
      titleRow.appendChild(dot);
    }
    ...
    state.className='session-attention-indicator session-state-indicator'+(paintStreaming?' is-streaming':(hasUnread?' is-unread':''))+attentionDotClass;
    state.setAttribute('aria-hidden','true');
    ...
    const _stateTip=_sessionStateTooltip({isStreaming:paintStreaming,hasUnread});
