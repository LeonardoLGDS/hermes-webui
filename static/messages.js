...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('interim_assistant',e=>{
      ...
      // Leo 2026-08-24: threshold was 3, so long agent turns hid almost all
      // progress notes (the final message reads differently from the live
      // updates, and on Telegram every note stays visible as its own message).
      // Raised to 20: real progress stays visible; only extreme turns collapse.
      const INTERIM_COLLAPSE_THRESHOLD=20;
  ...
  try {
    ...
    es.addEventListener('bg_status', e => {
      try {
        ...
        if (evSid !== sid) return;
        // This EventSource exists only for the visible pane. Refreshing the
        // cached sidebar here also repaints this row; non-visible rows catch
        // up from their bg_active/runs_total fields on the next list render.
        if (typeof window._updateSessionBgState === 'function') {
          window._updateSessionBgState(evSid, {
