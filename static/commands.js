...
function _steerFallbackIsDeadRun(fallback){
  // 'no_cached_agent' means nothing is cached AND the session carries no live
  // stream id, so the client's busy state is stale. Treat it like a dead run:
  // clear S.busy/activeStreamId/INFLIGHT and let the recovery card send the
  // message as a new turn, instead of offering a steer retry that can never
  // succeed while the user sees a scary failure toast (nesquena/hermes-webui #5723).
  return fallback==='stream_dead'||fallback==='no_cached_agent';
}
