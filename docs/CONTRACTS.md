...
Persisted session sidecars suppress replay only at `Session.save()`, and only
when a message has nonblank `id` plus `timestamp` (or legacy `_ts`) and its
complete dictionary is canonically JSON-equal to an earlier row with that same
identity. Partial identities, payload variants, repeated text, and non-dict
rows remain distinct. Any guarded shrink must retain the existing atomic
`.json.bak` recovery path; if canonicalization or backup inspection fails, the
unguarded message list is preserved. Already-inflated sidecars must be repaired
only from an offline copy or verified backup; never bulk-rewrite live user data
without retaining and checking the source sidecar first.
