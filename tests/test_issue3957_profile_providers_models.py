...
def test_detached_worker_scope_binds_explicit_default_profile(monkeypatch):
    """An explicit default worker must not inherit a named process profile."""
...
def test_detached_worker_modern_home_override_preserves_process_home(monkeypatch, tmp_path):
    """Modern Agent home routing must not mutate process-global HERMES_HOME."""
    base = tmp_path / ".hermes"
    work_home = base / "profiles" / "work"
    work_home.mkdir(parents=True)
    monkeypatch.setattr(profiles, "_DEFAULT_HERMES_HOME", base)
    ...
    fake_override = types.SimpleNamespace(
        set_hermes_home_override=set_override,
        reset_hermes_home_override=reset_override,
    )
    monkeypatch.setattr(profiles, "_resolve_hermes_home_override", lambda: fake_override)
    monkeypatch.setattr(profiles, "_resolve_secret_scope_module", lambda: None)

    with profiles.profile_env_for_background_worker(
        "work", "test-worker", scope_skill_modules=False
    ):
        assert current_home["value"] == work_home
        assert os.environ["HERMES_HOME"] == str(base)
