...
class TestHandleChatSteerFallbacks:
    ...
    def test_persisted_stream_id_after_restart_still_queues(self, _clear_caches):
        """Cache miss plus a persisted active_stream_id that the local STREAMS
        table no longer knows (webui restart while a gateway run stays live)
        must return the queue-safe gateway_steer_queued fallback instead of
        no_cached_agent, so the frontend queues the message. The gateway may
        still own the live run. See nesquena/hermes-webui #5723."""
        from api.streaming import _handle_chat_steer
        sid, stream_id = "sid_restarted", "stream_restarted"
