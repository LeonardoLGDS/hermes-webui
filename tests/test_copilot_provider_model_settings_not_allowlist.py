...
def test_discovered_provider_metadata_map_preserves_live_catalog(monkeypatch, tmp_path):
    import api.config as config

    cfg = {
        "model": {"default": "claude-configured", "provider": "anthropic"},
        "providers": {
            "anthropic": {
                "models_discovered": True,
                "models": {
                    "claude-configured": {"context_length": 200000},
                },
            }
        },
    }
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["claude-configured", "claude-live"] if pid == "anthropic" else [],
    )

    assert config._models_config_is_allowlist(["claude-configured"]) is True
    assert config._models_config_is_allowlist({"claude-configured": {}}) is False
    assert config._models_config_is_allowlist(["claude-configured"], discovered=True) is False
    ...
    assert ids == ["claude-configured", "claude-live"]
