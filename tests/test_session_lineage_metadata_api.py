...
def test_all_sessions_tolerates_subsecond_compression_handoff_overlap(_isolate):
    conn = _ensure_state_db(_isolate)
    t0 = time.time() - 100
    try:
        _save_webui_session("lineage_api_overlap_root", title="Hermes WebUI", updated_at=t0)
        _save_webui_session("lineage_api_overlap_tip", title="Hermes WebUI #2", updated_at=t0 + 10)
        _insert_state_row(
            conn,
            "lineage_api_overlap_root",
            started_at=t0,
            ended_at=t0 + 5,
            end_reason="compression",
        )
        _insert_state_row(
            conn,
            "lineage_api_overlap_tip",
            parent="lineage_api_overlap_root",
            started_at=t0 + 4.9,
        )

        rows = {row["session_id"]: row for row in all_sessions()}

        tip = rows["lineage_api_overlap_tip"]
        assert tip.get("_lineage_root_id") == "lineage_api_overlap_root"
        assert tip.get("_lineage_tip_id") == "lineage_api_overlap_tip"
...
def test_all_sessions_keeps_materially_overlapping_compression_child_separate(_isolate):
    conn = _ensure_state_db(_isolate)
    t0 = time.time() - 100
    try:
        _save_webui_session("lineage_api_overlap_parent", title="Parent", updated_at=t0)
        _save_webui_session("lineage_api_overlap_child", title="Child", updated_at=t0 + 10)
        _insert_state_row(
            conn,
            "lineage_api_overlap_parent",
            started_at=t0,
            ended_at=t0 + 5,
            end_reason="compression",
        )
        _insert_state_row(
            conn,
            "lineage_api_overlap_child",
            parent="lineage_api_overlap_parent",
            started_at=t0 + 4.5,
        )

        rows = {row["session_id"]: row for row in all_sessions()}

        child = rows["lineage_api_overlap_child"]
        assert child.get("relationship_type") == "child_session"
        assert child.get("_parent_lineage_root_id") == "lineage_api_overlap_parent"
