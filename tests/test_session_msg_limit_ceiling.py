...
def _foreign_session_get(monkeypatch, query: str, messages: list[dict]):
    ...
    monkeypatch.setattr(
        ...
        lambda _sid, cli_meta=None: (synth, "synthesized"),
    )
    monkeypatch.setattr(
        routes,
        "_merge_cli_sidebar_metadata",
        lambda session, _meta: session,
    )
    monkeypatch.setattr(routes, "_session_bg_status_fields", lambda _sid: {})
    monkeypatch.setattr(
        routes,
        "j",
    ...
    routes.handle_get(
        object(),
        urlparse(f"/api/session?session_id=foreign-session&resolve_model=0&{query}"),
    )
    return captured["payload"]["session"]


def test_foreign_session_get_honors_msg_limit_and_sets_paging_flags(monkeypatch):
    messages = [
    ...
    monkeypatch.setattr(
        routes,
        "attach_todo_state",
        lambda _session, full_messages: todo_inputs.append(full_messages),
    )

    session = _foreign_session_get(monkeypatch, "messages=1&msg_limit=2", messages)
    ...
    assert session["_messages_offset"] == 4
    assert todo_inputs == [messages]


def test_foreign_session_get_msg_before_paging_reaches_offset_zero(monkeypatch):
    messages = [
        {"role": "user" if index % 2 == 0 else "assistant", "content": f"message-{index}"}
        for index in range(6)
    ]
    monkeypatch.setattr(routes, "attach_todo_state", lambda _session, _messages: None)

    newest = _foreign_session_get(monkeypatch, "messages=1&msg_limit=2", messages)
    middle = _foreign_session_get(
        monkeypatch,
        f"messages=1&msg_limit=2&msg_before={newest['_messages_offset']}",
        messages,
    )
    oldest = _foreign_session_get(
        monkeypatch,
        f"messages=1&msg_limit=2&msg_before={middle['_messages_offset']}",
        messages,
    )
    ...
    assert oldest["_messages_truncated"] is False
    assert oldest["_messages_offset"] == 0
