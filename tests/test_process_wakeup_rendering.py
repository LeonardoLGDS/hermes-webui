...
const occupancyStart = src.indexOf("const isProcessWakeup=_isProcessWakeupMessage(m);");
const occupancyUserStart = src.indexOf('const isUser=', occupancyStart);
const occupancyEnd = src.indexOf(';', occupancyUserStart);
if(occupancyStart === -1 || occupancyUserStart === -1 || occupancyEnd === -1){
  throw new Error('render occupancy classification missing');
}
const occupancyExpression = src.slice(occupancyUserStart + 'const isUser='.length, occupancyEnd);
function classifyOccupancy(m){
  const isProcessWakeup=_isProcessWakeupMessage(m);
  const isUser=eval(occupancyExpression);
  return {isProcessWakeup, isUser};
}
const mixedHistory = [
...
const mixedOccupancy = mixedHistory.map(classifyOccupancy);
...
process.stdout.write(JSON.stringify({
  ...
  virtualHeight,
  mixedOccupancy,
  mixedUserColumn: mixedHistory.filter(m => classifyOccupancy(m).isUser).map(m => String(m.content)),
...
def test_process_wakeup_uses_compact_status_row_not_normal_user_bubble():
    ...
    assert "const isUser=m.role==='user'&&!isProcessWakeup;" in ui
    assert "const isEditableUser=isUser&&rawIdx===lastUserRawIdx;" in ui
    ...
    assert "row.id=_userMessageDomId(rawIdx);" in process_branch
    assert "row.dataset.msgIdx=rawIdx;" in process_branch
    assert "row.dataset.messageAnchorKey=_messageViewportAnchorKeyForMessage(m);" in process_branch
    assert "row.dataset.role='process_wakeup';" in process_branch
...
def test_process_wakeup_does_not_occupy_user_column_between_real_users():
    result = _run_driver()

    assert result["mixedOccupancy"] == [
        {"isProcessWakeup": False, "isUser": True},
        {"isProcessWakeup": True, "isUser": False},
        {"isProcessWakeup": False, "isUser": True},
    ]
    assert result["mixedUserColumn"] == ["first real question", "second real question"]
