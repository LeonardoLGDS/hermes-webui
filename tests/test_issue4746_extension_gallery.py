...
def test_install_allows_leaf_dotfiles(monkeypatch, tmp_path):
    ext_dir, state_dir = _setup_ext_env(monkeypatch, tmp_path)
    import api.extensions as ext_mod

    files = {
        "dot-ext/manifest.json": json.dumps({"version": "1.0.0"}),
        "dot-ext/screenshots/.gitkeep": "",
        "dot-ext/.env.example": "TOKEN=replace-me",
    }
    zip_bytes = _make_zip(files)
    sha = hashlib.sha256(zip_bytes).hexdigest()
    ...
    result = ext_mod.install_extension(
        "dot-ext",
        "https://hermes-webui.github.io/exts/dot-ext.zip",
        sha,
    )

    assert result["installed"] is True
    assert (ext_dir / "dot-ext" / "screenshots" / ".gitkeep").exists()
    assert (ext_dir / "dot-ext" / ".env.example").read_text(encoding="utf-8") == "TOKEN=replace-me"
    manifest = ext_mod._load_install_manifest()
    assert "screenshots/.gitkeep" in manifest["installed"]["dot-ext"]["files"]
    assert ".env.example" in manifest["installed"]["dot-ext"]["files"]

    assert ext_mod.uninstall_extension("dot-ext") == {"uninstalled": True, "id": "dot-ext"}
    assert not (ext_dir / "dot-ext").exists()
