...
class TestSendBusyBranchDispatch:
    ...
    def test_side_work_commands_bypass_busy_message_mode(self):
        """Mid-turn side-work commands must execute instead of steering or queueing."""
        send_idx = MESSAGES_JS.find("async function send(")
        busy_start = MESSAGES_JS.find("S.busy||compressionRunning", send_idx)
        busymode_idx = MESSAGES_JS.find("_defaultMessageMode||'steer'", busy_start)
        intercept_block = MESSAGES_JS[busy_start:busymode_idx]

        assert "'btw'" in intercept_block
        assert "'background'" in intercept_block
        assert "await _bc.fn(_pc.args)" in intercept_block
