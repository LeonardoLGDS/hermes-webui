...
class TestAgentCacheCredentialPoolStability(unittest.TestCase):
    ...
    def test_dynamic_credential_signature_is_stable_without_invoking_source(self):
        import api.streaming as streaming

        class DynamicCredential:
            def __call__(self):
                raise AssertionError("dynamic credential must not be invoked for cache lookup")

            def __str__(self):
                raise AssertionError("dynamic credential must not be stringified for cache lookup")

            def __repr__(self):
                raise AssertionError("dynamic credential must not be repr'd for cache lookup")

        source = DynamicCredential()

        self.assertEqual(
            streaming._agent_cache_api_key_sig(source, None),
            'dynamic-credential',
        )
