...
def test_tool_complete_error_state_uses_structured_result_fields_only():
    from api.streaming import _tool_result_is_error

    failed_results = [
        {"success": False, "error": "HTTP 433: usage allowance exceeded"},
    ...
    successful_results = [
        {"success": True, "error": ""},
        {"ok": True, "status": "completed", "returncode": 0},
        {"message": "error rate is below threshold"},
        "tool output contains the word error but is not structured failure metadata",
    ]

    assert all(_tool_result_is_error(result) for result in failed_results)
    assert not any(_tool_result_is_error(result) for result in successful_results)

    src = _read("api/streaming.py")
    block = _function_block(src, "on_tool_complete")
    assert "result_is_error = _tool_result_is_error(function_result)" in block
    assert "'is_error': result_is_error" in block
