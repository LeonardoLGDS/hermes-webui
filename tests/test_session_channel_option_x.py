...
def _next_event_after_bg_status(queue):
    """Return the next completion frame, validating additive status frames."""
    while True:
        event_name, data = queue.get(timeout=2.0)
        if event_name != "bg_status":
            return event_name, data
        assert set(data) == {"session_id", "active", "runs"}
...
def test_emit_when_idle_between_turns_session_channel_delivers():
    ...
    try:
        ...
        try:
            bp._process_one(evt)
            event_name, data = _next_event_after_bg_status(q)
...
def test_emit_during_busy_turn_dual_emits_to_both():
    ...
    try:
        ...
        event_name, data = next(
            item for item in streams_received if item[0] != "bg_status"
        )
        assert event_name == "bg_task_complete"
        assert data["task_id"] == "proc-busy-1"
        assert data.get("event_id"), "emitter must stamp event_id (Q4 contract)"

        ev2, data2 = _next_event_after_bg_status(q)
